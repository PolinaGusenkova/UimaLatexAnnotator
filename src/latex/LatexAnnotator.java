...
public class LatexAnnotator extends JCasAnnotator_ImplBase {
	...
	private Pattern otherFunc = Pattern.compile("\\\\(deg|det|dim|exp|gcd|hom|inf|ker|lg|ln|Pr|lim|liminf|limsup|log|max|min|sup|sum|prod|coprod|int|iint|iiint|oint|oiint|oiiint|over|frac)");
	...
	private Pattern accentVar = Pattern.compile("\\\\(vec|bar|breve|tilde|check|hat|dot|ddot|Delta)\\s*?([A-Za-z]|\\{[A-Za-z]\\})"); //includes delta x
	private Pattern other = Pattern.compile("\\\\[A-Za-z]+");
	...
	public void process (JCas aJCas) {
		...
		while (matcher.find()) { 
			...
			if (docText.charAt(end) == ' ' || docText.charAt(end) == '{' || docText.charAt(end) == '\\') {
		  ...
		  // search
	    matcher = otherFunc.matcher(docText);
		while (matcher.find()) { 
			start = matcher.start();
			end = matcher.end();
			if (docText.charAt(end) == ' ' || docText.charAt(end) == '{' || docText.charAt(end) == '\\' || docText.charAt(end) == '^' || docText.charAt(end) == '_') {
				 // remember annotated symbols
				for (int i = start; i < end; i++) {
					annotated[i] = true;
				}
				 // create annotation
				OtherFunc annotation = new OtherFunc(aJCas);
			    annotation.setBegin(start);
			    annotation.setEnd(end);
			    annotation.setName(matcher.group());
			    annotation.addToIndexes();
